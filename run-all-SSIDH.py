...
from all_basic_model import train_basic_data
from all_incre_model import train_incra_data
...
def load_config():
    ...
    parser.add_argument('--max-iter-unseen', default=1000, type=int, help='Number of iterations.(default: 60)')
    ...
    parser.add_argument('--num-samples', default=5000, type=int, help='Number of sampling data points.(default: 2000)')
    parser.add_argument('--batch-size', default=250, type=int, help='Batch size.(default: 64)')
    parser.add_argument('--code-length', default=16, type=int, help='Binary hash code length.(default: 12)')
    parser.add_argument('--num-seen', default=50, type=int, help='未增量前的类别数')
    parser.add_argument('--is-incre', default=True, type=bool)
    parser.add_argument('--version', default="all-htanh-v1", type=str)
    ...
    args.version = 'c' + str(args.num_seen) + '-p' + '1' + \
...
def run():
    ...
    if not args.is_incre:
        ...
        SEEN_B= train_basic_data(model, criterion, query_dataloader, seen_mark_dataloader, seen_unmark_dataloader,retrieval_dataloader, args)
    ...
    if SEEN_B is None:
        print("加载basic数据.....")
        SEEN_B = torch.load(os.path.join(basic_path, 'seen_b.t'))
